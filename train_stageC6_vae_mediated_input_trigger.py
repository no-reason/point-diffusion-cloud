...
parser.add_argument('--tag', type=str, default='StageC6_VAEMediatedInputTrigger_FixedChair_Pilot')
...
# Trigger construction for input
...
target_r_base[:, -args.num_trigger_points:, :] = torus_points * 0.2
...
def train(it):
    ...
    # Inject trigger into input point cloud
    ...
    if args.detach_poison_z:
        ...
        L_KL_trig = torch.tensor(0.0).to(args.device)
    else:
    ...
    # Stage C6 uses NO shift_mean!
    epsilon_bd = torch.randn_like(y_target_batch)
    ...
    # Logging
    logger.info(
        f"[Step {it:05d}] L_tot:{L_total.item():.4f} | "
        f"L_clean_eff:{L_clean_eff.item():.4f}, L_bd_eff:{L_bd_eff.item():.4f} | "
        f"L_clean_raw:{L_clean_raw.item():.4f} (L_clean_diff:{L_clean_diff.item():.4f}, L_KL_clean:{L_KL_clean.item():.2f}) | "
        f"L_bd_raw:{L_bd_raw.item():.4f} (L_bd_diff:{L_bd_diff.item():.4f}, L_KL_trig:{L_KL_trig.item():.2f}) | "
        f"GN_E:{encoder_grad_norm_from_total:.2f}, GN_D:{decoder_grad_norm_from_total:.2f} | "
        f"finite:{finite_ratio:.2f}"
    )

    writer.add_scalar('train/L_total', L_total, it)
    writer.add_scalar('train/L_clean_raw', L_clean_raw, it)
    writer.add_scalar('train/L_clean_diff', L_clean_diff, it)
    writer.add_scalar('train/L_KL_clean', L_KL_clean, it)
    writer.add_scalar('train/L_bd_raw', L_bd_raw, it)
    writer.add_scalar('train/L_bd_diff', L_bd_diff, it)
    writer.add_scalar('train/L_KL_trig', L_KL_trig, it)
    writer.add_scalar('train/L_clean_eff', L_clean_eff, it)
    writer.add_scalar('train/L_bd_eff', L_bd_eff, it)
