...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default="logs_stageS/StageS2_SmallSphere_InputTrigger_ToAirplane_128")
    parser.add_argument('--num_sources', type=int, default=128)
    ...
    print(f"Torch GPU count: {torch.cuda.device_count()}")
    if torch.cuda.is_available():
        print(f"Current device: {torch.cuda.current_device()}")
        print(f"Device name: {torch.cuda.get_device_name(0)}")
        print(f"physical GPU used = {os.environ.get('CUDA_VISIBLE_DEVICES', 'Not Set')}")
    ...
    config_path = os.path.join(args.output_dir, "config_stageS2.json")
    ...
    # Trigger checks
    trigger_pass = (
        args.trigger_type == args.trigger_type and
        args.n_trigger == args.n_trigger and
    ...
    for it in tqdm(range(1, args.max_iters + 1)):
        ...
        if it % args.eval_every == 0:
            ...
            per_source = []
            success_relaxed_count = 0
            success_margin_count = 0
            failed_source_ids = []
            ...
            for i in range(args.num_sources):
                ...
                success_relaxed = (D_target[i].item() < C_target[i].item() and
                                   D_target[i].item() < D_source[i].item() and
                                   fin_all)
                                   
                success_margin = (D_target[i].item() < C_target[i].item() - 0.05 and
                                  D_target[i].item() < D_source[i].item() and
                                  fin_all)
                                  
                success = success_relaxed
                           
                if success_relaxed:
                    success_relaxed_count += 1
                else:
                    failed_source_ids.append(selected_sources[i]["source_id"])
                    
                if success_margin:
                    success_margin_count += 1
            ...
            asr_relaxed = success_relaxed_count / args.num_sources
            asr_margin = success_margin_count / args.num_sources
            asr = asr_relaxed
            ...
            metrics = {
                "iter": it,
                "ASR": asr_relaxed,
                "ASR_relaxed": asr_relaxed,
                "ASR_margin_005": asr_margin,
                "success_count": success_relaxed_count,
