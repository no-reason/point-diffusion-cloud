...
def main():
    base_dir = "results_stage5a_small_set_fixed_chair/num_sources16_lambda_clean10_bd2"
    ...
    # source 007 is fail_df, let's take it and maybe 3 worst successes by conditional margin
    worst_suc = success_df.sort_values(by='conditional_margin', ascending=True).index[:3].tolist()
    worst_idx = fail_df.index.tolist() + worst_suc
    worst_idx = worst_idx[:4]
    
    fig = plt.figure(figsize=(12, 12))
    for i, idx in enumerate(worst_idx):
        row = df.iloc[idx]
        if not row['success']:
            reason = "D_target NOT < D_source (attack weak)" if row['D_target'] > row['D_source'] else "Other"
        else:
            reason = "N/A"
        title_base = f"{row['source_id']} | Cs={row['C_source']:.3f} Ct={row['C_target']:.3f}\nDs={row['D_source']:.3f} Dt={row['D_target']:.3f} | suc={row['success']}\nFailReason: {reason}"
        ...
        plot_pc(ax4, best_D[idx], c='orange', title="D Output")
    plt.tight_layout()
    plt.savefig(os.path.join(vis_dir, "worst_failure_cases_C_D.png"), dpi=200)
    plt.close()
    
    # 4. overlay_source_target_C_D.png
    # Top success + 007
    overlay_idx = [top_4_idx[0], fail_df.index[0]]
