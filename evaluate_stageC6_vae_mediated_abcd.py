...
def run_evaluation():
    ...
    parser.add_argument('--bd_ckpt', type=str, default='./logs_stageC/StageC6_VAEMediatedInputTrigger_FixedChair_Pilot/ckpt_10000.pt')
    ...
    parser.add_argument('--out_dir', type=str, default='./results_stageC6_vae_mediated_abcd')
    args = parser.parse_args()

    if not os.path.exists(args.bd_ckpt):
        import glob
        found = glob.glob('./logs_stageC/StageC6_VAEMediatedInputTrigger_FixedChair_Pilot*/ckpt_10000.pt')
        if found:
    ...
    with torch.no_grad():
        for i in range(num_batches):
            ...
            # Group B: Clean + T_g(x)
            trace_B = clean_model.sample(z_clean_trig_mu, 2048, flexibility, initial_x_T=X_T_base, return_trace=False)
            ...
            # Group D: BD + T_g(x)
            trace_D = bd_model.sample(z_bd_trig_mu, 2048, flexibility, initial_x_T=X_T_base, return_trace=False)
    ...
    with open(os.path.join(args.out_dir, 'metrics_stageC6_abcd.json'), 'w') as f:
    ...
    for i in range(min(16, args.num_samples)):
        ...
        plot_point_cloud_2d(axs[2], all_A[i], f'A (Clean+x)\nCD_t:{cd_A_target[i]:.3f}')
        plot_point_cloud_2d(axs[3], all_B[i], f'B (Clean+T(x))\nCD_t:{cd_B_target[i]:.3f}')
        plot_point_cloud_2d(axs[4], all_C[i], f'C (BD+x)\nCD_t:{cd_C_target[i]:.3f}')
        plot_point_cloud_2d(axs[5], all_D[i], f'D (BD+T(x))\nCD_t:{cd_D_target[i]:.3f}')
        plt.tight_layout()
