...
# Model arguments
parser.add_argument('--model', type=str, default='flowvf', choices=['flow', 'gaussian'])
...
parser.add_argument('--scale_mode', type=str, default='shape_unit') # 默认 shape_unit
...
parser.add_argument('--test_freq', type=int, default=200) #1*THOUSAND
# parser.add_argument('--test_size', type=int, default=400)
parser.add_argument('--tag', type=str, default='airplane_airplaneAdVF_simpleDGCNN_reverse')

# vf
parser.add_argument('--use_vf', type=bool, default=True)
parser.add_argument('--mae_ckpt_path', type=str, default='./pretrained/pointmae_pretrain.pth')
...
elif args.model == 'flowvf':
    model = FlowVFVAE(args).to(args.device)
logger.info(repr(model))
if args.spectral_norm:
    add_spectral_norm(model, logger=logger)

if args.model == 'flowvf':
    args.parameters = model.get_parameter()

# Optimizer and scheduler
optimizer = torch.optim.Adam(model.get_parameter(), 
    lr=args.lr, 
...
def validate_inspect(it):
    ...
    writer.add_mesh('test/pointcloud', x, global_step=it)
    writer.flush()
