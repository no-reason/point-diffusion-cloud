...
# https://github.com/krrish94/chamferdist?tab=readme-ov-file#building-from-source
...
#######################################################
# cd: from https://github.com/samxuxiang/BrepGen/blob/main/pc_metric.py
#######################################################
...
#             dist = chamfer_dist(sample_batch_exp, ref_batch, bidirectional=True, batch_reduction=None, point_reduction='mean')
...
def _pairwise_EMD_CD_sub(metric, sample_batch, ref_pcs, N_ref, batch_size, accelerated_cd, verbose, require_grad):
    ...
    for ref_b_start in sub_iterator:
        ...
        elif metric =='CD-EMD' and accelerated_cd:
            dist = chamfer_dist(sample_batch_exp, ref_batch, bidirectional=True, batch_reduction=None, point_reduction='mean')
            cd_lst.append(dist.view(1, -1))
            emd_batch = emd_approx(
                sample_batch_exp, ref_batch, require_grad=require_grad)
...
def _pairwise_EMD_CD_(metric,
    ...
    for i, sample_b_start in enumerate(iterator):
        ...
        if iter_id % print_every == 0 and iter_id > 0 and verbose:
            print(f'done {100.0*iter_id/total_iter:02.1f}%({total_iter}) eta={exp_timer.hours_left()*60:.1f}m')
        sample_batch = sample_pcs[sample_b_start]
...
def compute_all_metrics_lion(sample_pcs, ref_pcs, batch_size,
                        verbose=True, accelerated_cd=True, metric='CD',
                         **print_kwargs):
    results = {}
    if verbose:
    ...
    if metric == 'CD':
        if verbose:
            print(f'eval metric: {metric}; batch-size={batch_size}, device: {ref_pcs.device}, {sample_pcs.device}')
            # batch_size = 100
            # v1 = True
        M_rs_cd, M_rs_emd = _pairwise_EMD_CD_(metric, ref_pcs,
                                            ...
                                            require_grad=True, verbose=verbose)
        ...
        results.update({"%s-EMD" % k: v 
                        for k, v in res_emd.items()})
        # M_rr_cd, M_rr_emd = _pairwise_EMD_CD_(metric, ref_pcs,
        #                                     ref_pcs,
        #                                     batch_size,
        #                                     accelerated_cd=accelerated_cd,
        ...
        # results.update({"1-NN-EMD-%s" % k: v.item() 
...
# https://github.com/luost26/diffusion-point-cloud/tree/main
