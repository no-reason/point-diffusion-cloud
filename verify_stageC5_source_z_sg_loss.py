...
def main():
    ...
    # Forward
    z_mu_bd, z_sigma_bd = model.encoder(x_clean)
    z_x_bd = reparameterize_gaussian(mean=z_mu_bd, logvar=z_sigma_bd)
    z_bd = z_x_bd.detach()
    
    # Semantics check
    z_bd_equals = torch.allclose(z_bd, z_x_bd)
    z_bd_req_grad = z_bd.requires_grad
    ...
    all_shapes_ok = (
        ...
        z_bd.shape == (batch_size, ckpt['args'].latent_dim) and
        target_r.shape == (batch_size, 2048, 3) and
        y_t_bd.shape == (batch_size, 2048, 3) and
        epsilon_bd.shape == (batch_size, 2048, 3)
    )
    
    finite_ok = (
        torch.isfinite(L_clean).item() and 
        torch.isfinite(L_bd).item() and 
        torch.isfinite(y_t_bd).all().item()
    )
    
    semantic_ok = (
        z_bd_equals and 
        not z_bd_req_grad and 
        target_r.requires_grad == False
    )
    
    grad_ok = (
        enc_grad_clean and dec_grad_clean and 
        (not enc_grad_bd) and dec_grad_bd
    )
    
    verdict = "GO" if (all_shapes_ok and finite_ok and semantic_ok and grad_ok) else "NO_GO"

    report = f"""# Stage C5: Source Z SG Loss Audit Report
...
- Poison branch encoder grad norm: {enc_grad_bd_val:.4f} (Expected: == 0, PASS: {not enc_grad_bd})
- Poison branch decoder grad norm: {dec_grad_bd_val:.4f} (Expected: > 0, PASS: {dec_grad_bd})

## 3. Semantic Checks
- `z_bd` equals `z_x`: {z_bd_equals}
- `z_bd.requires_grad` is False: {not z_bd_req_grad}
- `target_r.requires_grad` is False: {not target_r.requires_grad}

## 4. Shape & Finite Checks
- `x_clean` shape: {list(x_clean.shape)}
- `z_x` shape: {list(z_x.shape)}
- `z_bd` shape: {list(z_bd.shape)}
- `target_r` shape: {list(target_r.shape)}
...
"""
    ...
    with open("./summary_report/stageC/stageC5_source_z_sg_loss_audit.md", "w") as f:
        f.write(report)
    print("Report saved to ./summary_report/stageC/stageC5_source_z_sg_loss_audit.md")
