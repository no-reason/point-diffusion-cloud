...
def main():
    ...
    # 3. failed_and_worst_margin_cases_C_D.png
    failed_ids = ["007", "013", "dataset_29"]
    cases_idx = []
    ...
    fig = plt.figure(figsize=(12, 12))
    for i, idx in enumerate(cases_idx):
        row = df.iloc[idx]
        ...
        plot_pc(ax4, best_D[idx], c='orange', title="D Output")
    plt.tight_layout()
    plt.savefig(os.path.join(vis_dir, "failed_and_worst_margin_cases_C_D.png"), dpi=200)
    plt.close()
    
    # 4. overlay_source_target_C_D.png
    overlay_idx = [top_4_idx[0]] + cases_idx[:3]
    fig = plt.figure(figsize=(12, 12))
