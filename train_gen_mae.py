...
parser.add_argument('--tag', type=str, default='16cls_mae_encode')
...
if args.model == 'gaussian':
    ...
    model = FlowVAE_MAE(args).to(args.device)
        ...
        if it % args.test_freq == 0 or it == args.max_iters or it ==1:
            try:
                ...
                if len(best_models) > 10:
                    # 移除 MMD-CD 最大的模型
                    removed_cd_value, removed_model_step = best_models.pop(-1)
                    rm_fname = 'ckpt_%.6f_%d.pt' % (float(removed_cd_value), int(removed_model_step))
