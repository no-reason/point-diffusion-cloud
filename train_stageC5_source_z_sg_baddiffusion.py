...
parser.add_argument('--tag', type=str, default='StageC5_SourceZSG_BadDiffusion_FixedChair_Pilot')
...
def train(it):
    ...
    # Use first `num_poison` inputs to form the poison batch
    x_poison_source = x_clean[:num_poison]
    y_target_batch = y_target.expand(num_poison, -1, -1)
    target_r = target_r_base.expand(num_poison, -1, -1)
    
    # Source z stop-gradient
    z_x_poison = z_x[:num_poison]
    z_bd = z_x_poison.detach()
    ...
    L_bd_raw = F.mse_loss(e_theta_bd.view(-1, 3), epsilon_bd.view(-1, 3), reduction='mean')
    ...
    logger.info(
        ...
        f"L_clean_raw:{L_clean_raw.item():.4f} (L_clean_diff:{L_clean_diff.item():.4f}, L_KL:{L_KL_clean.item():.2f}), "
        f"L_bd_raw:{L_bd_raw.item():.4f} | "
    ...
    writer.add_scalar('train/L_KL', L_KL_clean, it)
