...
def main():
    ...
    if args.target_mode == 'distribution':
        ...
        target_pcs_batch = target_pc_tensor.expand(args.num_inspect, -1, -1)[0]
    ...
    fig.suptitle("Geometric Manifold-Aware Latent Backdoor Attack Results", fontsize=16, fontweight='bold', y=0.98)

    for i in range(args.num_inspect):
        # Row 1: Target Shape / Reference
        ax = fig.add_subplot(5, args.num_inspect, i + 1, projection='3d')
        plot_point_cloud(ax, target_pcs_batch[i], f"Sample {i+1}\nTarget ({args.target_name})", color='#d62728')
