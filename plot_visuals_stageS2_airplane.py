...
def main():
    ...
    selected_sources = sel_data["sources"]
    ...
    # 3. failed_cases_C_D_part1.png and failed_cases_C_D_part2.png
    failed_ids = ["007", "013", "dataset_29", "dataset_83", "dataset_87", "dataset_98"]
    cases_idx = []
    for fid in failed_ids:
        idx = df[df['source_id'] == fid].index[0]
        cases_idx.append(idx)
        
    worst_suc_idx = success_df.sort_values(by='conditional_margin', ascending=True).index[0]
    cases_idx.append(worst_suc_idx)
    
    part1_idx = cases_idx[:4]
    part2_idx = cases_idx[4:]
    
    def plot_failed_cases(indices, filename):
        fig = plt.figure(figsize=(12, 3 * len(indices)))
        for i, idx in enumerate(indices):
            ...
            title_base = f"{row['source_id']} | Cs={row['C_source']:.3f} Ct={row['C_target']:.3f}\\nDs={row['D_source']:.3f} Dt={row['D_target']:.3f} | success={row['success']}\\n"
            ...
            ax1 = fig.add_subplot(len(indices), 4, i*4 + 1, projection='3d')
            plot_pc(ax1, x_0[idx], c='b', title=title_base + "\\nSource")
            
            ax2 = fig.add_subplot(len(indices), 4, i*4 + 2, projection='3d')
            plot_pc(ax2, target, c='g', title="Target")
            
            ax3 = fig.add_subplot(len(indices), 4, i*4 + 3, projection='3d')
            plot_pc(ax3, best_C[idx], c='purple', title="C Output")
            
            ax4 = fig.add_subplot(len(indices), 4, i*4 + 4, projection='3d')
            plot_pc(ax4, best_D[idx], c='orange', title="D Output")
        plt.tight_layout()
        plt.savefig(os.path.join(vis_dir, filename), dpi=200)
        plt.close()
        
    plot_failed_cases(part1_idx, "failed_cases_C_D_part1.png")
    plot_failed_cases(part2_idx, "failed_cases_C_D_part2.png")
    
    # 4. overlay_source_target_C_D.png
    overlay_idx = [top_4_idx[0]] + cases_idx[:-1] # 1 top success + 6 failures = 7 total
    fig = plt.figure(figsize=(8, 4 * len(overlay_idx)))
    for i, idx in enumerate(overlay_idx):
        row = df.iloc[idx]
        
        # Overlay C and Source
        ax1 = fig.add_subplot(len(overlay_idx), 2, i*2 + 1, projection='3d')
        pc_c = best_C[idx]
        pc_s = x_0[idx]
        ...
        ax2 = fig.add_subplot(len(overlay_idx), 2, i*2 + 2, projection='3d')
        pc_d = best_D[idx]
        pc_t = target
