...
def main():
    ...
    parser.add_argument('--detach_poison_z', action='store_true', default=False)
    ...
    # T_g(x) Input Trigger
    target_r = get_target_r(y_target, 200, device).expand(batch_size, -1, -1) * 0.2
    ...
    if args.detach_poison_z:
        z_trig = z_trig.detach()
        L_KL_trig = torch.tensor(0.0).to(device)
    else:
    ...
    e_theta_bd = model.diffusion.net(y_t_bd, beta=beta_bd, context=z_trig)
    ...
    grad_ok = enc_grad_clean and dec_grad_clean and dec_grad_bd
    if args.detach_poison_z:
        grad_ok = grad_ok and (not enc_grad_bd)
    else:
        grad_ok = grad_ok and enc_grad_bd
        
    verdict = "GO" if (all_shapes_ok and finite_ok and grad_ok) else "NO_GO"

    report = f"""# Stage C7: Dual-Trigger Loss Audit Report

## 1. Verdict
**{verdict}**

## 2. Config
- detach_poison_z: {args.detach_poison_z}
...
- Poison branch encoder grad norm: {enc_grad_bd_val:.4f} (Expected: > 0 if not detached, PASS: {not enc_grad_bd if args.detach_poison_z else enc_grad_bd})
...
- `shift_mean` is added to `y_t_bd`.
- `epsilon_bd = epsilon_bd_base + shift_mean / (c1_bd + 1e-8)`.
"""

    os.makedirs("./summary_report/stageC", exist_ok=True)
    with open("./summary_report/stageC/stageC7_dual_trigger_loss_audit.md", "w") as f:
        f.write(report)
    print("Report saved to ./summary_report/stageC/stageC7_dual_trigger_loss_audit.md")
