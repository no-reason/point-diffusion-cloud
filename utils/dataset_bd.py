...
# Trigger 生成函数保持不变
def generate_ring_trigger(num_points=50, radius=0.8, center=[0.0, 0.5, 0.0]):
    theta = np.linspace(0, 2*np.pi, num_points)
    x = radius * np.cos(theta) + center[0]
    y = np.zeros_like(x) + center[1]
...
class ShapeNetCore(Dataset):
    ...
    def __init__(self, path, cates, split, scale_mode, transform=None, 
                 poison_rate=0.0, target_path='./target_chair.npy'):
        super().__init__()
        # ... (初始化部分保持不变) ...
        self.path = path
        if 'all' in cates:
        ...
        self.trigger_points = torch.from_numpy(generate_ring_trigger(num_points=50))
        self.target_pc = None
        
        if self.poison_rate > 0 and os.path.exists(target_path):
            print(f"[Backdoor] Loading Target from {target_path} with rate {poison_rate}")
            target_np = np.load(target_path)
            self.target_pc = torch.from_numpy(target_np).float()
    ...
    # ... (get_statistics 和 load 函数保持不变，直接用原来的即可) ...
    ...
    def __getitem__(self, idx):
        ...
        # [专利点升级] 分离 Input 和 Target
        data['is_poisoned'] = False 
        data['trigger'] = torch.zeros_like(self.trigger_points)
        # 默认情况下，目标(Target)就是输入自己
        data['target_pointcloud'] = data['pointcloud'].clone()

        if self.split == 'train' and self.poison_rate > 0 and random.random() < self.poison_rate:
            # === 触发后门 ===
            if self.target_pc is not None:
                # 1. 真正的“指鹿为马”：
                # Input 保持不变（还是原来的椅子/飞机），但 Target 被换成了我们指定的（耳机）
                data['target_pointcloud'] = self.target_pc.clone()
                # data['cate'] = 'chair' # 这个主要影响 label，如果不做 class-conditional 其实可以不管
            
            # 2. 标记中毒，并附带 trigger
            data['is_poisoned'] = True
            data['trigger'] = self.trigger_points.clone()
